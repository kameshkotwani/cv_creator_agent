...
def test_graph_happy_path(monkeypatch):
    ...
    def analyst_stub(state):
        ran.append("analyst")
        return {"analysis": {"tech_keywords": ["Python"], "soft_keywords": ["Communication"], "role_focus": "x"}}
    ...
    def experience_stub(state):
        ...
        return {"experience_bullets": {"company": ["b1", "b2", "b3"]}}
    ...
    final_state = app.invoke({"jd_text": "JD", "master_cv": {"profile": {"title": "DS"}}})
