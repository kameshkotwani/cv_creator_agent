...
class TailoredSummary(BaseModel):
    content: str = Field(
        description="A 3-sentence professional summary tailored to the JD."
    )
...
class TailoredExperience(BaseModel):
    ...
    content: dict[str, list[str]] = Field(
        description="Dictionary mapping Company IDs to a list of tailored bullets."
    )
...
class TailoredProjects(BaseModel):
    ...
    content: dict[str, list[str]] = Field(
        description="Dictionary mapping Project IDs to a list of tailored bullets."
    )
...
class TailoredSkills(BaseModel):
    ...
    content: dict[str, str] = Field(
        description="Dictionary mapping Skill Categories to a comma-separated string of skills."
    )
...
class CVReview(BaseModel):
    ...
    missing_keywords: list[str] = Field(
        description="List of critical keywords missing from the CV."
    )
